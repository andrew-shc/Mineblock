...
impl Cube {
    ...
    pub fn new_all(coord: [u16; 2]) -> Cube {
        Cube {
            top: coord,
            bottom: coord,
            left: coord,
    ...
    pub fn new_single(all: [u16; 2], unique: [u16; 2], face: CubeFace) -> Cube {
        Cube {
            top: if face == CubeFace::TOP {unique} else {all},
            bottom: if face == CubeFace::BOTTOM {unique} else {all},
            left: if face == CubeFace::LEFT {unique} else {all},
            right: if face == CubeFace::RIGHT {unique} else {all},
            front: if face == CubeFace::FRONT {unique} else {all},
            back: if face == CubeFace::BACK {unique} else {all}
        }
    }
}
...
impl Renderable for Cube {
    ...
    fn ind_data(&self, index: u32) -> Vec<u32> {
        vec![  0, 1, 2, 0, 2, 3,
               4, 5, 6, 4, 6, 7,
               8, 9,10, 8,10,11,
               12,13,14,12,14,15,
